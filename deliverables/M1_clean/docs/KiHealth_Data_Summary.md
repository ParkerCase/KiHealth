...
- **NIHANES 2017–20:** DEMO + GLU + INS (+ BMX) → 5,090 with fasting glucose + insulin. *Merge on SEQN:* SEQN is the NHANES respondent sequence number (unique ID per person per cycle). We join demographics (DEMO), glucose (GLU), insulin (INS), and body measures (BMX) by SEQN so each row is one person with all variables.  
