...
# Verify structure (OAI only: 4796; OAI+CHECK: 5798)
assert df.shape[0] >= 4796, f"Wrong number of rows: {df.shape[0]} (expected ≥4796)"
...
n_total = len(df)
print(f"✓ Train set: {X_train.shape[0]} samples ({X_train.shape[0]/n_total*100:.1f}%)")
print(f"✓ Test set: {X_test.shape[0]} samples ({X_test.shape[0]/n_total*100:.1f}%)")
...
- **Train set:** {X_train.shape[0]} ({X_train.shape[0]/n_total*100:.1f}%)
- **Test set:** {X_test.shape[0]} ({X_test.shape[0]/n_total*100:.1f}%)
