...
# ============================================================================
# STEP 5b: Append CHECK cohort (OAI + CHECK for Models 1 & 2)
# ============================================================================
print("\n" + "=" * 80)
print("STEP 5b: Appending CHECK Cohort (OAI + CHECK pooled)")
print("=" * 80)

check_oai_path = base_path / "data/New-OA-Data/check_oai_format.csv"
if check_oai_path.exists():
    check_df = pd.read_csv(check_oai_path)
    # Ensure same columns (order and names) as baseline_data
    for col in baseline_data.columns:
        if col not in check_df.columns:
            raise ValueError(f"CHECK missing column: {col}")
    check_df = check_df[baseline_data.columns]
    n_oai = len(baseline_data)
    baseline_data = pd.concat([baseline_data, check_df], axis=0, ignore_index=True)
    print(f"  ✅ Appended CHECK: {len(check_df)} rows (OAI: {n_oai}, total: {len(baseline_data)})")
    print(f"  CHECK 4yr TKR events: {check_df['knee_replacement_4yr'].sum()}")
else:
    print(f"  ⚠ CHECK file not found at {check_oai_path}; run scripts/build_check_oai_dataset.py first. Using OAI only.")
...
# Check 1: At least 4,796 rows (OAI); may be 5,798 if CHECK appended)
check1 = len(baseline_data) >= 4796
validation_results.append(("≥4,796 rows (OAI or OAI+CHECK)", check1))
print(
    f"  {'✅' if check1 else '❌'} Merged dataset has {len(baseline_data)} rows (expected ≥4796)"
)
