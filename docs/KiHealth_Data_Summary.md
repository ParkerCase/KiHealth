...
- **NIHANES 2017–20:** DEMO + GLU + INS (+ BMX) → 5,090 with fasting glucose + insulin (merge on SEQN).  
- **NIHANES 2021–23:** DEMO + GLU + INS (+ BMX) → 3,996 (merge on SEQN).  
...
1. **Frankfurt:** Glucose=0 or Insulin=0 in some rows → invalid for HOMA; exclude or flag.  
2. **DiaBD:** “Skin Thickness(mm)” values 259–381 mm → likely unit/coding error; verify before use.  
3. **NHANES:** LBDINLC has anomalous tiny values (e.g. 5.4e-79); use **LBXIN** (μU/mL) for HOMA.  
4. **October 2025 Blood Drive:** Many biomarker/methylation columns empty → wait for complete data or use only A1c for descriptive work.

---

## 9. Recommendations for Preprocessing

1. **Merge NHANES:** DEMO + GLU + INS on SEQN per cycle; keep WTSAFPRP/WTSAF2YR for weighted analyses.  
2. **Unit consistency:** Use mg/dL and μU/mL for HOMA; convert mmol/L → mg/dL where needed.  
3. **Frankfurt/DiaBD:** Drop or impute rows with Glucose ≤ 0 or Insulin ≤ 0 for HOMA; document exclusions.  
4. **Identifiers:** Use SEQN (NHANES) or row index; never log or export patient names/emails (HIPAA).  
5. **Methylation:** Once KiHealth methylation files are finalized, link by sample_id/patient_id and document CpG/feature set in a separate data spec.
