...
export async function GET() {
  try {
    ...
    const records = await xata.db.target_rankings
      .select(['*'])
      .getAll();

    return NextResponse.json(records);
  } catch (error: any) {
    console.error('Error fetching target rankings:', error);
    return NextResponse.json(
      { error: 'Failed to fetch target rankings', details: error?.message },
