...
export async function GET() {
  try {
    ...
    const records = await xata.db.synthetic_lethality
      .select(['*'])
      .getAll();

    return NextResponse.json(records);
  } catch (error: any) {
    console.error('Error fetching synthetic lethality:', error);
    return NextResponse.json(
      { error: 'Failed to fetch synthetic lethality', details: error?.message },
