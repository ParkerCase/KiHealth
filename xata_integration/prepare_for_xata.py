...
for _, row in rankings_df.iterrows():
    ...
    summary_parts = []
    summary_parts.append(f"{row['cancer_type']} ranks #{row['rank']} with overall score {row['overall_score']:.3f}")
    ...
    if row['validation_data_available']:
        summary_parts.append(f"IC50 validation available ({int(row['n_validated_cell_lines'])} lines)")
    
    if row['significant_SL_hits'] > 0:
        summary_parts.append(f"{int(row['significant_SL_hits'])} significant synthetic lethality hits")
    ...
    for target in ['STK17A', 'MYLK4', 'TBK1', 'CLK4']:
        ...
        if score < -0.15:
            target_scores.append(f"{target} shows strong dependency (score: {score:.3f})")
        elif score < -0.08:
            target_scores.append(f"{target} shows moderate dependency (score: {score:.3f})")
    ...
    record = {
        ...
        "n_validated_lines": int(row['n_validated_cell_lines']) if pd.notna(row['n_validated_cell_lines']) else 0,
        "validation_available": bool(row['validation_data_available']),
        "significant_sl_hits": int(row['significant_SL_hits']) if pd.notna(row['significant_SL_hits']) else 0,
...
try:
    ...
    for _, row in significant_sl.head(500).iterrows():  # Limit to top 500 for now
        # Parse cell lines
        mutant_lines = row['mutant_cell_lines'].split(', ') if pd.notna(row['mutant_cell_lines']) else []
        ...
        if mutant_lines:
            summary += f"Most dependent mutant line: {row['most_dependent_mutant_cell']}"
        ...
        record = {
            ...
            "most_dependent_line": str(row['most_dependent_mutant_cell']) if pd.notna(row['most_dependent_mutant_cell']) else "",
